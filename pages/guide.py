...
データを用いています。正確な食事療法については、必ず主治医や管理栄養士の指導に従ってください。
""")
...
st.subheader("4. iphoneのホーム画面にアプリを登録する")
st.write("""
1. iPhoneでSafariアプリ を開きます。
2. 目的のWebサイトに移動します。
3. その他ボタン(・・・)をタップしてから、「共有」をタップします。タブレイアウトが「下」または「上」の場合、共有ボタン をタップします。Safariでレイアウトを変更するを参照してください。
4. オプションのリストを下にスクロールしてから、「ホーム画面に追加」をタップします。「ホーム画面に追加」が表示されない場合は、このオプションを追加できます。リストの一番下までスクロールし、「アクションを編集」をタップしてから「 ホーム画面に追加」をタップします。
